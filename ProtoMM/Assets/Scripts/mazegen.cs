        ...
        for (int i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (int j = 0; j <= Maze.GetUpperBound(1); j++)
            {
                if (Maze[i, j] == 1)
                {
                    ...
                    ptype.gameObject.tag = "stop";
                    ptype.transform.localScale = new Vector3(5,20,5);
                ...
                {
                        flat = GameObject.CreatePrimitive(PrimitiveType.Cube);
                        flat.gameObject.tag = "stop";
                    flat.transform.localScale = new Vector3(5,1,5);
                        flat.transform.position = new Vector3(i * flat.transform.localScale.x, 0, j * flat.transform.localScale.z);
                    MazeString = MazeString + "."; // added to create String

                    if (floorColor != null) { flat.GetComponent<Renderer>().material = floorColor; }
